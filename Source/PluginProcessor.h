...
struct SingleChannelBuffer
{
    SingleChannelBuffer(Channel ch) : channelToUse(ch)
    {
        prepared.set(false);
    }
...
private:
    ...
    int fifoIndex = 0;
    Buffer<BlockType> audioBufferFifo;
...
class ProvaDSPAudioProcessor  : public juce::AudioProcessor
{
public:
    ProvaDSPAudioProcessor();
    ~ProvaDSPAudioProcessor() override;
    ...
    SingleChannelBuffer<BlockType> leftChannelFifo { Channel::Left };
    SingleChannelBuffer<BlockType> rightChannelFifo { Channel::Right };
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (ProvaDSPAudioProcessor)
};
